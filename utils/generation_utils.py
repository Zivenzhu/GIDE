...
def sine_schedule(t: torch.Tensor) -> torch.Tensor:
    """Cosine schedule function: m(t) = sin(π/2 · t) """
...
def gumbel_max_sample(logits: torch.Tensor, tau: float = 1.0, *, generator: Optional[torch.Generator] = None) \
    ...
    result = (logits / tau + g)
    return result.argmax(dim=-1), result

def mask_by_random_topk_inversion(
    ...
    *,
    ...
    for i in range(B):
        k = int(mask_len[i].item())
        k = max(0, min(k, L)) 
        if k > 0:
            _, idx = torch.topk(confidence[i], k, largest=False)
            mask[i, idx] = True
