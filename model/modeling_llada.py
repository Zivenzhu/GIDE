...
class LLaDABlock(nn.Module):
    ...
    def attention(
        self,
            use_attention_control: bool,
            target_object_indices: List,
            image_mask,
            att_score_list,
        q: torch.Tensor,
        k: torch.Tensor,
        ...
        if use_attention_control and att_score_list.get_state():
            q_text = q[:, :, target_object_indices]
            k_img = k[:, :, image_mask]
            att_t2i = q_text @ k_img.transpose(-2, -1)

            q_img = q[:, :, image_mask]
            k_text = k[:, :, target_object_indices]
            att_i2t = q_img @ k_text.transpose(-2, -1)
            att_i2t = att_i2t.transpose(-2, -1)

            att_score_list.add_attention_score(att_t2i)
            att_score_list.add_attention_score(att_i2t)
...
class LLaDAModelLM(PreTrainedModel):
    ...
    def forward(
        ...
        outputs = self.model.forward(
            ...
            added_hidden_state=added_hidden_state,
